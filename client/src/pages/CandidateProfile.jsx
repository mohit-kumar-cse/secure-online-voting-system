...
const CandidateProfile = () => {
  ...
  useEffect(() => {
    const fetchCandidate = async () => {
      try {
        const { data } = await axios.get(`http://localhost:5000/api/candidates/${id}`);
        setCandidate(data);
      } catch (err) {
    ...
    <div className="max-w-xl mx-auto py-8 px-4">
      <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
        ...
        <div className={`relative bg-gradient-to-br ${colors.bg} px-6 pt-10 pb-20 text-center overflow-hidden`}>
          ...
          <div className="relative inline-block mb-4">
            ...
            <div className="absolute -bottom-2 -right-2 bg-white rounded-full px-2 py-1 flex items-center gap-1 shadow-md">
              ...
              <span className="text-xs font-bold text-blue-700">{candidate.totalVotes}</span>
            </div>
          </div>
        ...
        <div className="px-5 pb-6 space-y-3">
          ...
          <div className="border border-gray-100 rounded-2xl p-4 hover:border-green-100 hover:bg-green-50/30 transition">
            ...
            <div className="space-y-2">
              {goals.map((goal, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 bg-gray-50 hover:bg-green-50 hover:translate-x-1 rounded-xl px-3 py-2.5 transition-all duration-200 cursor-default"
                >
          ...
          {error && (
            <p className="text-xs text-red-500 text-center bg-red-50 border border-red-100 rounded-xl py-2.5">
              {error}
            </p>
          )}
